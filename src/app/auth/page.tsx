...
export default function AuthPage() {
  ...
  useEffect(() => {
    if (!googleClientId) return; // no client id configured -> no Google button
    ...
    const renderGoogle = () => {
      ...
      if (el) {
        el.innerHTML = "";
        g.accounts.id.renderButton(el, { theme: "outline", size: "large", width: 340, text: "continue_with" });
      }
    };
        ...
        <div className="w-full" style={{ maxWidth: "440px" }}>
          ...
          {activeTab === "login" && (
            <form onSubmit={handleLoginSubmit}>
              ...
              {googleError && (
                <p style={{ color: "#dc2626", fontSize: 13, marginTop: 8, textAlign: "center" }}>{googleError}</p>
              )}
            </form>
          )}
