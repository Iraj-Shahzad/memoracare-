...
**Dataset:** `data/intents.json` — **14 intents**, **~202 example phrases**, in **English + Urdu** (greeting, medication_query, routine_query, family_query, emergency, etc.).
...
- **Small ML dataset** (~202 phrases) → high accuracy but limited generalisation to very different phrasings. *Mitigation:* confidence threshold + rule-based fallback; more data would help.
...
19. *What accuracy did you get?* → **[fill in from your `train.py` run]** on the held-out test set, with a classification report + confusion matrix.
