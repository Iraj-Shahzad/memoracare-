 ...
 *
 * Key concepts: Job A (runs every minute) fires real-time "due now" reminders by
 * comparing each active Medication time / Routine startTime against the current minute, emits a
 * 'reminder' to the patient's socket room, and logs a delivered Reminder. Job B (runs every 10
 * minutes) sweeps for misses: once a scheduled time is past by GRACE_MINUTES (30) with no
...
function isMedicationActiveToday(med: any, now: any) {
...
async function emitDueReminders(io: any) {
  const now = new Date();
...
async function detectMissed(io: any) {
  const now = new Date();
  const nowMinutes = now.getHours() * 60 + now.getMinutes();
...
export { start, parseTimeToMinutes };
